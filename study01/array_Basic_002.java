...
// 이 무제는 자바의 배열 및 인덱스 사용법 그리고 배열 생성시 초깃값이 어떻게 정해지는지를 묻는 문제이다.


// [1]: 배열과 인덱스
// 크기 10의 정수형 배열을 선언하고 크기 10을 저장하면 메모리상에는 10개의 정수형 값을 담을 수 있는 공간이 생성된다.
// 배열 변수는 참조형 타입으로써 해당 메모리 공간을 가리키게 된다. --> 즉, 해당 메모리 공간의 주소 값을 가진다. --> 이 주소 값을 통해서 참조한다.
// 주소 값은 다른 표현으로는 참조 값이라고도 부른다.
// 공간이 10개가 만들어지면 동시에 자동적으로 해당 공간에 접근할 수 있는 인덱스 번호가 자동 부여된다. --> 인덱스 번호는 0부터 시작.
// 따라서, 크기가 10인 배열 공간의 첫 번째 인덱스는 0이고, 마지막 공간의 인덱스는 9가 된다. --> 즉, 마지막 인덱스는 "배열의 크기-1"이 된다.
// 인덱스 사용 --> 배열명[인덱스번호]

public class array_Basic_002 {
    public static void main(String[] args) {
        ...
        System.out.println(scores[s_size-1]);   // 0 (마지막 인덱스)
    }
}
