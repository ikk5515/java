...
(2) 클래스선언 --> class
            --> 객체를 생성하는 틀, 프레임, (객체를 만들어내는) 공장, 템플릿
...
(1) 메인메서드 --> main() --> 만약 다르게 작성하면 기본 메서드를 찾을 수 업다라고 에러 발생 --> 기본 메서드(main) 작성하라고 함.
...
(2) 파라미터스 --> 메서드(함수) 호출시 하나 or 둘 이상의 파라미터 값을 넣어서 호출할 수 있음.
            --> 그러한 인수(파라미터, 매개변수)들의 값을 저장할 변수(바구니)들을 명시.
            --> String --> 문자열 --> [] 배열 --> (보통)args --> argument(논의,논쟁,언쟁,주장; 인수, 독립 변수)
