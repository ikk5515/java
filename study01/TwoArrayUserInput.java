...
// 이 문제는 배열 생성시 사용사로 부터 입력을 받아서 2차원 배열을 만들 수 있는지를 묻는 문제이다.
...
public class TwoArrayUserInput {
    public static void main(String[] args) {
        ...
        // [2]: 2차원 배열의 행과 열 선언 --> 사용자 입력 처리
        System.out.print("행의 갯수를 입력하고 [Enter] 치세요 = ");
        int Row = sc.nextInt();
        System.out.print("열의 갯수를 입력하고 [Enter] 치세요 = ");
        int Column = sc.nextInt();

        // [3]: char 2차원 배열 선언 --> 사용자로부터 Row, Column 입력받아 --> gameMap(Row)(Column) 생성.
        char[][] gameMap = new char[Row][Column];

        // [4]: 사용자 입력을 받아서 저장할 String 배열 선언 --> 이때, 크기룰 [Row]로 해주도록 한다.
        // 크기를 Row 만큼만 해주면 되는 이유? --> 어차피 사용자가 입력하는 값은 안쪽 for 문을 돌면서 2차원 배열에 입력하기 때문이다.
        String[] strAr = new String[Row];

        // [5]: 2차원 배열 요소 --> 사용자가 입력하는 값으로 셋팅
        for(int i = 0; i < Row; i++) {
            System.out.print((i+1) +"번째 행에 입력할 문자"+ Column +"개를 차례대로 입력하고 [Enter] 치세요 = ");
            strAr[i] = sc.next();
